...
with ExitStack(lambda: sys.path.pop(0)):
    from SimpleSchema.Schema.Impl.Parse import AntlrException
...
class TestMultilineStringErrors(object):
    ...
    def test_InvalidOpeningToken(self):
        with pytest.raises(
            AntlrException,
            match=re.escape("Triple-quote delimiters that initiate multiline strings cannot have any content on the same line. (root_file [2, 27])"),
        ):
            Test(
                textwrap.dedent(
                    """\
    ...
    def test_InvalidClosingToken(self):
        with pytest.raises(
            AntlrException,
            match=re.escape("Triple-quote delimiters that terminate multiline strings cannot have any content on the same line. (root_file [4, 24])"),
        ):
            Test(
                textwrap.dedent(
                    """\
    ...
    def test_InvalidIndentation(self):
        with pytest.raises(
            AntlrException,
            match=re.escape("Invalid multiline string indentation. (root_file [4, 20])"),
        ):
            Test(
                textwrap.dedent(
                    """\
